...
export function ScaleSlider({ value, onChange, isOpen, onToggle }: ScaleSliderProps) {
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className="flex items-center gap-2 px-3 py-2 rounded-full bg-secondary/80 backdrop-blur-sm border border-border/50 hover:bg-secondary transition-colors"
      >
        <Search className="w-4 h-4 text-muted-foreground" />
        <span className="text-xs text-muted-foreground font-medium">Zoom In/Out</span>
      </button>
